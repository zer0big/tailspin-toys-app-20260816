...
- **Client-side catalog filters** — accessible title search, category, and publisher controls narrow the prerendered game cards in place without a runtime API or UI framework.
...
Repository-wide contribution guidance lives in [`.github/copilot-instructions.md`](.github/copilot-instructions.md), with file-specific standards in [`.github/instructions/`](.github/instructions/). In particular:

- Comments explain intent, constraints, and non-obvious decisions rather than restating code, and they must be updated or removed when behavior changes.
- Every exported function in `db/` and `src/lib/` has TSDoc/JSDoc covering its purpose, parameters, and return value.
- Reusable Astro components document their `Props` interfaces and non-obvious properties.
- TypeScript uses 4-space indentation, single quotes, semicolons, and trailing commas in multiline structures. ESLint is the source of truth for enforceable TypeScript quality rules.
...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) loads the repository's open GitHub issues through the GitHub CLI, highlights the three issues that need attention first, and groups the remaining issues into a backlog. Each issue card can add its full context to the current Copilot session and start the work. Contributors must authenticate the GitHub CLI with access to this repository before opening the canvas.
